...
void key_setting_page_rever_period_handle(uint8_t direction)
{
	...
	save_set_data.rever_period_sum_s = save_set_data.rever_period_m * 60 + save_set_data.rever_period_h * 3600;
}
...
void key_ok_down_entry_callback(void *arg)
{
	beep_send(BEEP_MSG_TYPE_1);
...
{
        pwr_msg_send(PWR_MSG_TYPE_WAKE_UP);
}
...
void button_entry(void *arg)
{
	...
	Button_Attach(&key_entry, BUTTON_DOWM, key_ok_down_entry_callback);
