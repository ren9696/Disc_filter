...
//static rt_uint32_t save_flash_default_id = 0;
struct save_set_data_t save_set_data;
 
#if 1
#define SAVE_THREAD_PRIORITY 	13
...
#define SAVE_MSG_POOL_SIZE (sizeof(enum save_msg_type_t) * 5)
...
{
	if (rt_mq_send(&save_mq, &msg, sizeof(enum save_msg_type_t)) != RT_EOK){
		return -1;
	}
